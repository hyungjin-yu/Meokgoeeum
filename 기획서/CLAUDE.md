...
- `2026-08-21_6층콘텐츠-준비.md` — 코드 변경 없음(`RoomClearGate`가 `EnemyBun` 미니언 분열까지 이미 감안한 설계였음). [[logic/6층 콘텐츠 - 비 오는 거리 씬 설정]] 신규(면3 배정 — **채워지면 6면 전부 완성되는 마일스톤**). **Unity 작업 아직 안 함**
...
- **Unity에서 6층 콘텐츠 구성 필요**: `EnemyBun` 프리팹 최초 생성 → `SC_Face_5` 새 씬 → Addressable+`CubeFaceData_5`+`GameState.faceData[3]` 연결 → 평×2/분×1 배치 → `RoomClearGate`+`Stairs` (→ [[logic/6층 콘텐츠 - 비 오는 거리 씬 설정]] 0~5단계). **이번이 마지막 남은 면(면3) — 채워지면 큐브 6면이 처음으로 전부 완성됨**
