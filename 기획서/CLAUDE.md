...
- [x] **흡(EnemyHeup) 회복 히스테리시스 + 공격 능력 추가** — 회복 시작하면 100%까지 계속 회복(사용자 피드백: "왜 절반까지만 회복해?"), HP 50% 이상이면 평과 동일한 근접 공격 가능(사용자 요청). ⚠️ `27 전투 프레임 데이터`의 "비공격 유닛" 서술과 어긋남 — 문서 업데이트 아직 안 함. **Unity 테스트 아직 안 함**
...
- `2026-08-20_흡-회복개선-공격추가.md` — `EnemyHeup` 회복 히스테리시스(100%까지 계속 회복) + HP 50% 이상이면 근접 공격 가능(평과 동일 프레임). ⚠️ `27 전투 프레임 데이터`의 "비공격 유닛" 서술과 어긋남(문서 미수정). **Unity 테스트 아직 안 함**
