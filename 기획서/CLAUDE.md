...
- **1층 "방A(전투)" 완료 — 다음은 2~11층 콘텐츠**: [[19 층별 상세 설계]] + [[28 레벨 디자인 - 인카운터 페이싱과 블록아웃]]의 층별 웨이브/공간 스펙을 그대로 `EncounterSpawner` 데이터로 옮기면 됨(스크립트 인프라는 이미 재사용 가능). 1층의 "방B(색 구슬 파동 트리거)"·"입장 복도(튜토리얼 유도)"도 아직 별도 콘텐츠 작업 필요
