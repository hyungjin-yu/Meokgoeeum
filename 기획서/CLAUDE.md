...
- `2026-08-25_1층콘텐츠-입장복도.md` — 1층 "입장 복도"(1~2단계 이동/구르기 유도)+"방B"(색 복원 연출) 착수. `MovingObstacle.cs`/`TutorialCheckpoint.cs` 신규 + 씬 배치 가이드 문서. **스코프 컷: 팝업 힌트 UI/발자국 이펙트/ESC 스킵창/Lock-On은 안 함**(팝업 UI 시스템 자체가 없음). 코드만 완료, **Unity 씬 배치/검증은 아직 안 함**
...
- ⚠️ **[2026-08-25 코드+가이드 문서 작성 완료 + 컴파일 확인, Unity 씬 배치/검증 대기]** 1층 "입장 복도"(1~2단계: 이동/구르기 유도) + "방B"(색 복원 연출) — `MovingObstacle.cs`/`TutorialCheckpoint.cs` 신규, [[logic/1층 콘텐츠 - 입장 복도 & 방B 씬 설정]] 가이드 문서 작성 완료, 컴파일 에러 없음 확인. **스코프 컷: 팝업 힌트 UI·발자국 이펙트·ESC 스킵창·Tab Lock-On은 오늘 안 함**(팝업 UI 시스템 자체가 없어서, 별도 항목으로 추적 필요). 가이드 문서 보고 직접 씬 배치 진행할 것. 근거: [[changelog/2026-08-25_1층콘텐츠-입장복도]]
